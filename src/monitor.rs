...
) -> impl warp::Filter<Extract = (Arc<RwLock<RecognitionResponse>>,), Error = std::convert::Infallible>
       + Clone {
...
mod tests {
    ...
    fn test_database_monitor_creation() {
        ...
        let face_db = crate::database::FaceDatabase::new().unwrap_or_else(|_| {
            // If there's an error reading the database, create an empty one
            crate::database::FaceDatabase {
                records: Vec::new(),
            }
        });
        let monitor = DatabaseMonitor::new(face_db).unwrap_or_else(|e| {
            // If there's an error creating the monitor, panic with a clearer message
            panic!("Failed to create DatabaseMonitor: {}", e);
        });

        // The monitor may detect existing files in the database directory
        // We just verify it doesn't crash and creates a monitor object
        assert!(monitor.photo_files.len() >= 0);
    }
