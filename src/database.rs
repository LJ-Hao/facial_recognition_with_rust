...
mod tests {
    ...
    fn test_face_database_operations() {
        ...
        // Create a new empty database file to avoid parsing errors
        fs::write(test_db_path, "[]").unwrap();
        ...
        // Verify record was added (account for any existing records)
        assert!(face_db.records.len() >= initial_count);
        // Find our specific record
        let added_record = face_db.records.iter().find(|r| r.name == "Jane Doe");
        assert!(added_record.is_some());

        // Test retrieving authorized faces
        let authorized_faces = face_db.get_authorized_faces();
        assert!(authorized_faces.len() >= 1);
        let found_record = authorized_faces.iter().find(|r| r.name == "Jane Doe");
    ...
    fn test_face_database_save_load() {
        ...
        // Ensure test file doesn't exist before saving
        if Path::new(test_db_path).exists() {
            let _ = fs::remove_file(test_db_path);
        }
        ...
        // Verify the loaded database has at least the records we added
        assert!(loaded_db.records.len() >= 2);
